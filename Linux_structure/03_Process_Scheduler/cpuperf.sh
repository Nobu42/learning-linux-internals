...
usage() {
    ...
    3. 同、スループットのグラフを作って'throoughput.jpg'に保存

    -m オプションはmultiload.shプログラムにそのまま渡す"
    exit 1
}
...
measure() {
    local nproc=$1
    local opt=$2
    bash -c "time ./multiload.sh $opt $nproc" 2>&1 | grep real | sed -n -e 's/^/*0m\([.0-9]*\)s$/\1p' | awk -v nproc=$nproc '
BEGIN{
    sum_tat=0
}
(NR==nproc) {
